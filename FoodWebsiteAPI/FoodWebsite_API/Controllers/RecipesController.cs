...
namespace FoodWebsite_API.Controllers
{
    ...
    public class RecipesController : ControllerBase
    {
        ...
        public async Task<ActionResult<IEnumerable<RecipeSummaryDTO>>> GetRecipes([FromQuery] RecipeFilterDTO filter)
        {
            ...
            {
                // Case-insensitive search giống như SpecialtiesController
                string normalizedQuery = filter.SearchTerm.Trim().RemoveDiacritics();
                string rawQueryLower = filter.SearchTerm.Trim().ToLowerInvariant();
                
                query = query.Where(r => r.Name.ToLower().Contains(rawQueryLower) ||
                                         (r.NamePlain != null && r.NamePlain.Contains(normalizedQuery)) ||
                                         (r.Description != null && r.Description.ToLower().Contains(rawQueryLower)));
            }
