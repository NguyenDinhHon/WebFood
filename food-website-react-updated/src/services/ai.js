  ...
  try {
    const res = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key=${geminiKey}`, {
      ...
      body: JSON.stringify({
        contents: [{
          parts: [{
            text: `Bạn là trợ lý ẩm thực Việt Nam chuyên nghiệp với kiến thức sâu về:
...
Hãy trả lời câu hỏi sau một cách ngắn gọn (2-4 câu), thân thiện và hữu ích. Nếu người dùng hỏi về món ăn, hãy:
1. Giới thiệu món ăn và xuất xứ
2. Mô tả nguyên liệu chính
3. Gợi ý cách ăn hoặc biến tấu (nếu phù hợp)
...
Câu hỏi: ${message}`
    ...
    if (!res.ok) {
      ...
      // If API fails, fallback to intelligent mock
      console.warn('⚠️ Gemini API failed, using intelligent mock response');
      const category = classifyMessage(message);
      const responses = mockResponses[category];
