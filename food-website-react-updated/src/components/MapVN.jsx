...
export default function MapVN({ onProvinceClick }) {
  ...
  // Dữ liệu tỉnh chia theo 3 miền (bỏ Tây Nguyên)
  ...
  const loadProvinces = async () => {
    try {
      // Lấy dữ liệu từ API
      const response = await provincesAPI.getAllProvinces()
      if (response && Array.isArray(response)) {
        setProvinces(response)
      } else {
        // Fallback về dữ liệu cứng nếu API không trả về
  ...
  if (loading) return <div style={{ padding: '20px', textAlign: 'center' }}>Đang tải danh sách tỉnh thành...</div>
